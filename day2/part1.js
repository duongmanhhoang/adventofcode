...
input.split('\n').forEach(line => {
  const arrayResult = line.split(': ');
  const id = parseInt(arrayResult[0].split(' ')[1]);
  let isValidId = true;
  
  arrayResult.pop().split('; ').forEach((colorPack) => {
    colorPack.split(', ').forEach((color) => {
    	const arrColor = color.split(' ');
      const num = arrColor[0];
      const colorStr = arrColor[1];
      
      switch(colorStr) {
      	case "red":
        	if (num > MAX_RED) isValidId = false;
          break;
        case "green":
        	if (num > MAX_GREEN) isValidId = false;
          break;
        case "blue":
        	if (num > MAX_BLUE) isValidId = false;
          break;
         default:
         	break;
      }
